...
function seedDemoData() {
  ...
  state.devices = [
    { id: uid(), networkId: lan.id, name: 'Routeur fibre', type: 'Routeur', ip: '192.168.1.1', mac: '00:11:22:33:44:55', status: 'Actif', location: 'Baie réseau', notes: 'Passerelle et DNS local.' },
    { id: uid(), networkId: lan.id, name: 'NAS', type: 'Serveur', ip: '192.168.1.10', mac: 'AA:BB:CC:DD:EE:10', status: 'Actif', location: 'Baie réseau', notes: 'SMB, sauvegardes, monitoring.' },
    { id: uid(), networkId: iot.id, name: 'Thermostat', type: 'IoT', ip: '192.168.30.21', mac: 'AA:BB:CC:30:00:21', status: 'À vérifier', location: 'Couloir', notes: 'Vérifier mises à jour firmware.' },
    { id: uid(), networkId: guest.id, name: 'Téléphone invité', type: 'Mobile', ip: '192.168.50.42', mac: 'AA:BB:CC:50:00:42', status: 'Réservé', location: 'Wi-Fi', notes: '' }
  ];
  saveState();
}
...
function filteredDevices() {
  ...
  return state.devices.filter((d) => {
    const haystack = normalize([d.name, d.type, d.ip, d.mac, d.status, d.location, d.notes, networkName(d.networkId)].join(' '));
...
function renderCard(d) {
  return `<article class="device-card"><span class="badge">${networkName(d.networkId)}</span><span class="badge ${statusClass(d.status)}">${d.status}</span><h3>${d.name}</h3><p><strong>${d.type}</strong> · ${d.location || 'emplacement non défini'}</p><p>IP: ${d.ip || '-'}<br>MAC: ${d.mac || '-'}</p><p>${d.notes || ''}</p><div class="card-actions"><button onclick="editDevice('${d.id}')">Éditer</button><button class="danger" onclick="deleteDevice('${d.id}')">Supprimer</button></div></article>`;
}
function renderTable(devices) {
  const rows = devices.map((d) => `<tr><td>${d.name}</td><td>${networkName(d.networkId)}</td><td>${d.type}</td><td>${d.ip || '-'}</td><td>${d.mac || '-'}</td><td><span class="badge ${statusClass(d.status)}">${d.status}</span></td><td><button onclick="editDevice('${d.id}')">Éditer</button> <button class="danger" onclick="deleteDevice('${d.id}')">Supprimer</button></td></tr>`).join('');
  return `<table><thead><tr><th>Nom</th><th>Réseau</th><th>Type</th><th>IP</th><th>MAC</th><th>Statut</th><th>Actions</th></tr></thead><tbody>${rows}</tbody></table>`;
}
...
$('networkForm').addEventListener('submit', (event) => {
  ...
  event.target.reset(); $('networkId').value = ''; saveState();
});
$('deviceForm').addEventListener('submit', (event) => {
  event.preventDefault();
  const id = $('deviceId').value || uid();
  const record = { id, networkId: $('deviceNetwork').value, name: $('deviceName').value.trim(), type: $('deviceType').value, ip: $('deviceIp').value.trim(), mac: $('deviceMac').value.trim().toUpperCase(), status: $('deviceStatus').value, location: $('deviceLocation').value.trim(), notes: $('deviceNotes').value.trim() };
  const index = state.devices.findIndex((d) => d.id === id);
  index >= 0 ? state.devices.splice(index, 1, record) : state.devices.push(record);
  event.target.reset(); $('deviceId').value = ''; saveState();
});
window.editNetwork = (id) => { const n = state.networks.find((item) => item.id === id); if (!n) return; $('networkId').value = n.id; $('networkName').value = n.name; $('networkCidr').value = n.cidr; $('networkZone').value = n.zone; $('networkNotes').value = n.notes; scrollTo({ top: 0, behavior: 'smooth' }); };
